...
author: "Josh Nugent, Fall 2019, updated Spring 2023"
output:
  html_document:
...
Heavily indebted to various internet sources; see References at the end for the most prominent. Additional thanks to [Dr. Mara Tableman](https://www.pdx.edu/profile/exists?id=bymt) for corrections via email.
...
knitr::opts_chunk$set(fig.height=4, fig.width=7, fig.align='center', warning = F, message = F, cache = T)
set.seed(1)
```
...
### Penalization: Restrict the $b_i$ coefficient magnitudes
...
betas_.5 <- solve(t(X) %*% X + (.5) * D) %*% t(X) %*% y
...
ggplot() +
  ...
  labs(title = "lambda = 0 (red, overfit), .5 (black, good), 9 (purple, underfit)")
```
